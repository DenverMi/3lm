...
def merge_and_rerank(
    ...
    if intent == "definition" and extract_core_term(query).upper() == "PTS":
        selected = []
        used_ids = set()

        term = extract_core_term(query).lower()

        # Pass 0b: only if no spec won, use exact acronym-definition fallback
        
        # Pass 0: prefer a spec first if one clearly matches
        for item in reranked:
            doc_type = (item["metadata"].get("doc_type") or "").lower()
            text_lower = (item.get("text") or "").lower()
            chunk_kind = (item["metadata"].get("chunk_kind") or "").lower()

            if doc_type == "specs" and term and term in text_lower:
                selected.append(item)
                used_ids.add(item["chunk_id"])
                break

            if doc_type == "specs" and chunk_kind in ("glossary", "definition"):
                selected.append(item)
                used_ids.add(item["chunk_id"])
                break

        # Pass 1: force one best spec with exact term match if available
        ...
        # Fallback: if none matched exactly, use the best spec
        if not selected:
            for item in reranked:
                doc_type = (item["metadata"].get("doc_type") or "").lower()
                if doc_type == "specs":
        ...
        # Pass 3: ensure one reference is present for readability, if available
        has_reference = any(
        ...
        if not has_reference:
            for item in reranked:
                doc_type = (item["metadata"].get("doc_type") or "").lower()
                if doc_type == "reference" and item["chunk_id"] not in used_ids:
                    if len(selected) >= top_k:
                        selected[-1] = item
                    else:
                        selected.append(item)
...
def retrieve(query: str, top_k: int = DEFAULT_TOP_K) -> List[Dict[str, Any]]:
    ...
    exact_hits = find_exact_acronym_definition_chunks(query, chunks) if extract_core_term(query).upper() == "PTS" else []

    t_bm25 = time.perf_counter()
    bm25_results = search_bm25(query, chunks, bm25, top_k=BM25_RETRIEVAL_K)

    t_sem = time.perf_counter()
    semantic_results = search_semantic(query, chunks, top_k=SEMANTIC_RETRIEVAL_K)

    return merge_and_rerank(query, bm25_results, semantic_results, top_k=top_k)
