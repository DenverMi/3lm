Read the attached MD file and extract retrieval-optimized reusable Matter consulting cases from it.
...
* One thread may contain one or more distinct Matter consulting cases.
...
* If the thread contains no reusable Matter consulting knowledge, output an empty cases array.

Your goal:

* Understand the thread
* Identify each distinct reusable Matter certification / compliance / testing / technical / procedural issue
...
9. Do not infer missing emails, attachments, screenshots, test reports, certificates, CSA records, test harness logs, DCL records, or technical evidence that are not explicitly visible in the thread.
...
12. Prefer Matter certification, compliance, test, and implementation meaning over generic business summarization.
...
* program:
  Always return "Matter".

* iut_type:
  The product / implementation under test if inferable.
  Examples: "Matter device", "controller", "commissioner", "bridge", "hub", "lighting device", "switch", "sensor", "lock", "thermostat", "plug", "gateway", "border router", "Wi-Fi device", "Thread device", "Ethernet device".
  If unclear, use "".
...
* actual_issue:
  The real Matter certification / compliance / testing / technical / procedural problem behind the question.
...
* decision_logic:
  ...
  Focus on the rule, dependency, mismatch, eligibility condition, test requirement, evidence requirement, certification principle, device type requirement, cluster/PICS requirement, or procedural condition that drives the answer.
...
* program_tags:
  ...
  Examples:
  "Matter", "Alliance", "CSA", "certification", "test harness", "PICS", "PIXIT", "device type", "cluster", "endpoint", "attribute", "command", "commissioning", "controller", "commissioner", "bridge", "bridged device", "DCL", "DAC", "PAI", "CD", "attestation", "vendor ID", "product ID", "software version", "firmware update", "Thread", "Wi-Fi", "Ethernet", "border router", "test report", "test log", "declaration", "listing", "post-certification change"
...
* search_aliases:
  ...
  Examples:
  "Matter certification issue"
  "CSA certification question"
  "Matter test harness failure"
  "PICS mismatch"
  "PIXIT issue"
...
* If the thread is mostly about quotation / scheduling but still includes a real Matter certification, compliance, testing, or implementation decision, extract that decision as a case.
...
* If a thread contains no reusable Matter consulting issue, return:
  {
