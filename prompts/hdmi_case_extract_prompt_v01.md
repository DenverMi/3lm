Read the attached MD file and extract retrieval-optimized reusable HDMI consulting cases from it.
...
* One thread may contain one or more distinct HDMI consulting cases.
...
* If the thread contains no reusable HDMI consulting knowledge, output an empty cases array.

Your goal:

* Understand the thread
* Identify each distinct reusable HDMI compliance / certification / testing / technical / procedural issue
...
9. Do not infer missing emails, attachments, screenshots, test reports, certificates, CTS results, ATC records, or technical evidence that are not explicitly visible in the thread.
...
12. Prefer HDMI compliance, certification, test, and implementation meaning over generic business summarization.
...
17. Do not invent risks, costs, timelines, or test counts unless explicitly supported by the thread or directly implied by the compliance or testing outcome.
...
* iut_type:
  ...
  Examples: "HDMI source", "HDMI sink", "HDMI repeater", "TV", "monitor", "projector", "AV receiver", "set-top box", "streaming device", "game console", "adapter", "cable", "switch", "splitter", "capture device".
  If unclear, use "".
...
* actual_issue:
  The real HDMI compliance / certification / testing / technical / procedural problem behind the question.
...
* decision_logic:
  ...
  Focus on the rule, dependency, mismatch, eligibility condition, test requirement, evidence requirement, compliance principle, certification principle, or procedural condition that drives the answer.
...
* risk_if_done_wrong:
  Explain the likely failure, delay, rework, rejection, unnecessary testing, compliance risk, certification risk, or customer-facing risk if the issue is handled incorrectly.
...
* program_tags:
  ...
  Examples:
  "HDMI", "HDMI LA", "HDMI Forum", "CTS", "ATC", "compliance testing", "test report", "test log", "source", "sink", "repeater", "cable", "adapter", "connector", "HDCP", "EDID", "CEC", "eARC", "ARC", "FRL", "TMDS", "VRR", "ALLM", "HDR", "HDMI 2.0", "HDMI 2.1", "logo usage", "declaration", "listing", "certification", "interoperability", "firmware change", "design change"
...
* If the thread is mostly about quotation / scheduling but still includes a real HDMI compliance, certification, testing, or implementation decision, extract that decision as a case.
...
* If a thread contains both administrative discussion and a real technical, compliance, certification, or testing issue, keep only the reusable consulting issue as a case.
* If a thread contains no reusable HDMI consulting issue, return:
  {
