...
- [Optional] Aliro Bluetooth-LE only expedited standard flow & thus the Bluetooth-LE only Aliro flow declared in section [11.1](#page-98-1)

![](_page_18_Picture_11.jpeg)

# <span id="page-19-0"></span>**2 References[1](#page-19-3)**
...
The access protocol used for the transaction proceeds in two phases as shown in [Figure 5-1:](#page-25-3)

An **expedited** phase (see [8.1.1\)](#page-48-2) in which the User Device proves to the Reader possession of a given secret key. If the Reader can make an access decision based on possession of this key, it can indicate success to the User Device and terminate the transaction.

<span id="page-25-3"></span>A **document presentation** phase (see section [8.4\)](#page-87-0), denoted as "step-up", in which the User Device provides an Access Document, that is bound to the Access Credential public key, to the Reader which contains detailed information about the Access Credential that the Reader can use to make this access decision. If access is granted, the Reader MAY cache the Access Credential public key along with any associated access rights given in that document to ensure that subsequent transactions can be resolved during the expedited phase.
...
A transaction can take place using either NFC or Bluetooth LE as the transport mechanism for the access protocol. When performing the transaction over Bluetooth LE, one of the flows uses UWB to securely determine proximity as specified in section [11.](#page-98-0)
...
The data in an Access Document and Revocation Document are contained in data elements. If the Reader requests an Access Document or Revocation Document, it has to be provisioned with a list of data element identifiers to use in the DeviceRequest structure, see section [8.4.1.](#page-87-1)
...
A Reader SHALL support reader\_PubK authentication method as defined in section [8.3.3.4.](#page-71-0)

A Reader key pair SHALL be ECC P-256 as specified in [\[5\].](#page-19-6)

# <span id="page-28-1"></span>**6.3.1 Reader certificate**

The reader\_Cert is an X.509 certificate issued by the Reader System Issuer CA and SHALL follow the format as defined in section [13.1.](#page-164-1)

A Reader SHALL support reader\_Cert authentication method as defined in section [8.3.3.3.](#page-70-0)
...
If the reader\_Cert is present on the Reader, it SHALL be presented to the User Device using either LOAD CERT or AUTH1 command (see section [8.3.3.3](#page-70-0) and section [8.3.3.4\)](#page-71-0).

The reader\_Cert SHALL be presented in compressed format as defined in section [13.3.](#page-165-1) The format in which the reader\_Cert is stored on the Reader is out of scope of this specification.

The User Device SHALL accept at least 274 bytes of reader\_Cert transmitted as compressed format when either the LOAD CERT (see section [8.3.3.3](#page-70-0) ) or the AUTH1 command (see section [8.3.3.4\)](#page-71-0) is sent.
...
An Access Document SHALL be signed by the Credential Issuer private key (IssuerKey\_PrivK). Optionally the Credential Issuer public key (IssuerKey\_PubK) is contained in the Credential Issuer Certificate (see section [6.5.1\)](#page-29-2) issued by the Credential Issuer CA.
...
The Issuer\_Cert is an X.509 certificate issued by the Credential Issuer CA and SHALL follow the format as defined in section [13.1.](#page-164-1)
...
When the Reader is not connected to a backend or panel, the revocation information can be transported from a User Device to the Reader using the mechanisms described in section [7.6](#page-44-1) and section [8.4.](#page-87-0)
...
As part of the step-up phase (see section [5.2\)](#page-25-2), an Access Document or Revocation Document (see section [7.6\)](#page-44-1) can be securely transmitted from the User Device to the Reader. When a User Device presents an Access Document to a Reader, the Reader can use the information from the Access Document to help inform its access decision. This Access Document consists of data signed by a Credential Issuer. A Credential Issuer can vary greatly in the number of Access Credentials that it manages, ranging from a single Access Credential in a household or an Access Credential for each employee in a global enterprise. A Reader can be setup to trust credentials from one or multiple Credential Issuers.

After the Reader has determined that it trusts the Credential Issuer (see section [6\)](#page-27-0) that has issued the Access Document, it will look at the data elements and the signatures within that Access Document. The data elements will provide information to the Reader to inform its access decision.
...
Section [8.4](#page-87-0) describes how data elements are requested and transmitted.
...
- The x5chain field in the header of IssuerAuth as specified in section 9.1.2 of [\[6\].](#page-19-7) This field MAY be present. If present this field SHALL contain the Credential Issuer certificate as defined in [6.5.1.](#page-29-2)
...
An additional field SHALL be present in the MobileSecurityObject with the key as "TimeVerificationRequired" and the boolean field TimeVerificationRequired. The meaning of this field is defined in section [7.2.4.](#page-34-0)

An additional field MAY be present in the "validityInfo" field in the MobileSecurityObject with the key as "validityIteration" and the integer field ValidityIteration. The meaning of this field is defined in section [7.2.3.](#page-33-0)

To optimize the size of the IssuerAuth structure the keys in the maps SHALL be replaced by a different key (note that these are integers, still encoded as text strings) according to [Table 7-1.](#page-33-1) Some of the keys are nested, shown by indentations.
...
To optimize the size of the IssuerSignedItem structure the keys in the maps SHALL be replaced by a different key (note that these are integers, still encoded as text strings) according to [Table 7-2.](#page-33-2)
...
The DocType and Namespace for the Access Document and Revocation Document are defined in [7.7.](#page-47-1)
...
Note that section [7.2](#page-31-2) requires all CBOR structures to be encoded according to the core deterministic requirements in section 4.2.1 of [\[27\].](#page-19-8)
...
Note that while this requirement means that support of the content for certain structures is optional, the processing rules as defined in [7.5](#page-44-0) always apply. This is especially relevant for the requirements of rejecting the whole Access Data Element when encountering certain unknown structures like an unknown access extension.
...
The fields in the AccessRuleCapabilitiesBits as defined in the CDDL have the meaning as defined in [Table 7-3.](#page-38-1)
...
- 4. Verify that the DocType in the IssuerAuth structure matches the relevant DocType in the Documents structure in the DeviceResponse, see section [8.4.2.](#page-88-0)
- 5. The following time-based elements in the ValidityInfo structure SHALL be validated considering the TimeVerificationRequired element (see section [7.2.4\)](#page-34-0):
...
- 6. If present, a Reader SHALL verify the ValidityIteration field as defined in section [7.2.3.](#page-33-0)
...
- Process the Version field according to section [7.3.1.](#page-36-0)
- If present, process AccessRule structures according to section [7.3.3.](#page-36-2)
- If present, process the ReaderRuleId structure according to section [7.3.5.](#page-41-0)
- If present, process the AccessExtension structures according to section [7.3.7.](#page-42-0)

Information from an Access Data Element SHALL NOT be used if it is considered invalid.

A Reader MAY process non-access extensions. If it does, it SHALL process them according to section [7.3.6.](#page-41-1)

# <span id="page-44-1"></span>**7.6 Revocation Document**

This specification defines a Revocation Document that contains revocation information that the Reader can use to inform its access decision. The Revocation Document can be retrieved using the same mechanism used to retrieve the Access Document, see section [8.4.](#page-87-0)
...
- The DocType SHALL be the doctype for the Revocation Document as defined in [7.7.](#page-47-1)
...
This specification defines one data element for the Revocation Document, the Revocation Data Element defined in section [7.6.1.](#page-45-0) The Revocation Document SHALL contain one or more Revocation Data Elements.

The Revocation Data Element SHALL have the NameSpace as defined in section [7.7.](#page-47-1)
...
The Access Protocol described in this section is independent of which transport protocol is used. The different transport protocols are defined in section [9.](#page-91-0)
...
[Figure 8-1](#page-50-0) illustrates the flow of commands for the expedited-standard phase, it illustrates the scenario that results in a successful transaction.
...
<span id="page-51-0"></span>![](_page_51_Figure_3.jpeg)
...
[Figure 8-2](#page-51-0) illustrates the flow of commands for the expedited-fast phase. It illustrates the scenario that results in a successful transaction.
...
[Figure 8-3](#page-52-2) illustrates the flow of commands for the step-up phase. It illustrates the scenario that results in a successful transaction.
...
- **Step 1:** The Access Protocol flow starts with the Reader and User Device setting up the physical and transport layer according to section [9.](#page-91-0)
- **Step 2:** The Reader sends the AUTH0 command, see section [8.3.3.2.1.](#page-64-0) The command parameters determine if the Reader initiates the expedited-standard phase or expedited-fast phase. The User Device responds with the AUTH0 response, see section [8.3.3.2.2.](#page-65-0)
...
- **Step 3:** In this OPTIONAL step, the Reader sends the LOAD CERT command, see section [8.3.3.3.1](#page-70-1) and the User Device sends the LOAD CERT response, see section [8.3.3.3.2.](#page-70-2)
- **Step 4:** The Reader sends the AUTH1 command, see section [8.3.3.4.1](#page-71-1) and the User Device sends the AUTH1 response, see section [8.3.3.4.2.](#page-72-0)
- **Step 5:** In this OPTIONAL step, the Reader and User Device exchange one or more EXCHANGE commands, see section [8.3.3.5.1,](#page-79-0) and EXCHANGE responses, see section [8.3.3.5.2.](#page-85-0)
...
- **Step 6:** In this OPTIONAL step, the Reader and User Device exchange one or more ENVELOPE and GET RESPONSE messages and its corresponding ENVELOPE responses and GET RESPONSE responses, see section [8.4.4,](#page-90-0) as part of the step-up phase.
- **Step 7:** In the OPTIONAL step, the Reader and User Device exchange one or more EXCHANGE command, see section [8.3.3.5.1,](#page-79-0) and EXCHANGE responses, see section [8.3.3.5.2.](#page-85-0)

Note that a transport protocol can have additional requirements on the order and optionality of certain steps.

If the User Device or Reader receives a message that does not follow the flow as required in this section it is considered a failure, and the failure process as defined in section [8.3.3.1](#page-62-1) SHALL be executed.
...
Note that while the X9.63 key derivation function is used in the procedure to calculate Kdh, actual key derivation is performed using the procedure defined in section [8.3.1.5.](#page-54-0)
...
SHALL generate 160 bytes of derived key material derived\_keys\_fast according to [8.3.1.5](#page-54-0) using Kpersistent as input\_key\_material, info, and salt\_fast as salt.
...
SHALL generate 160 bytes of derived key material derived\_keys\_volatile according to section [8.3.1.5](#page-54-0) using Kdh, info and salt\_volatile.
...
- Generate 32 bytes of derived key material derived\_keys\_persistent according to section [8.3.1.5](#page-54-0) using Kdh, info and salt\_persistent.
...
During the Expedited phase the Reader SHALL indicate a user authentication policy in the AUTH0 command (see section [8.3.3.2.1\)](#page-64-0). The user authentication policy indicates the Reader intent for the user authentication policy to be applied on the User Device.

The User Device will apply either a user-defined configurable policy or a hard-coded policy based on the received user authentication policy as per [Table 8-1.](#page-59-0) If the Reader indicates the "Force user authentication" policy, the User Device SHALL perform User Authentication or terminate the transaction.

The Reader SHOULD NOT use the "Force user authentication" – 0x03 option when performing the Bluetooth LE + UWB Aliro Flow as defined in section [11.1.1](#page-98-2) in a passive entry flow.
...
The data content of the mailbox can be read/written in any order using offset/length parameters with atomicity guarantee as described in section [8.3.3.5.](#page-78-0) The offsets are relative to the start of the mailbox memory space.
...
The mailbox data format is defined in appendix [18.](#page-187-0)

The mailbox access rights (read/write/none) SHALL be configurable and updatable by the Credential Issuer. The configuration method is out of scope of this specification. Access rights for the currently authenticated Reader are indicated during transaction in the signaling bitmap as per [Table 8-11.](#page-72-1)
...
Read and write access to mailbox data at reader-chosen offsets is provided by the EXCHANGE command as per section [8.3.3.5.](#page-78-0) In addition, it is possible to configure an Access Credential such that the AUTH1 response returns a pre-defined subset of the mailbox. This can be useful to read a status or a table of content without an EXCHANGE roundtrip.
...
For each response in the expedited phase, this specification specifies the response data field. General requirements for the status bytes are defined in section [8.3.2.3.](#page-61-0)
...
- Return an error code as defined in section [8.3.2.3.](#page-61-0)
- Destroy all session-bound keys and data
- Terminate the transaction.

When this happens to the Reader, it SHALL:

- Perform Reader behavior described in [Table 8-2.](#page-63-0)
- Terminate the transaction.
- Destroy all session-bound keys and data
...
The cryptogram plain\_payload SHALL be formatted according to [Table 8-6.](#page-66-0)
...
Vendor extensions MAY not be interpreted by the receiver but SHALL always be included in the cryptography flow as per section [8.3.1.12](#page-56-0) and section [8.3.1.13](#page-57-0) when received.
...
The Reader SHALL generate the AUTH0 command message according to [Table 8-3](#page-64-1) and [Table 8-4.](#page-65-2)
...
The User Device SHALL execute the failure process as defined in section [8.3.3.1](#page-62-1) if one of the following scenarios occurs:

- The AUTH0 command is not received as per sequencing flow described in section [8.2.](#page-52-1)
- The AUTH0 command is not formatted according to [Table 8-3](#page-64-1) and [Table 8-4.](#page-65-2)
...
The User Device SHALL conditionally perform user authentication based on authentication\_policy and following the requirements from section [8.3.1.14.](#page-58-0)
...
- The User Device SHALL execute the key material generation procedure defined in section [8.3.1.12.](#page-56-0)
- The User Device SHALL compute encrypted\_payload and authentication\_tag as per [8.3.1.10](#page-56-1) using cryptogramSK as key and plain\_payload as payload.
...
The User Device SHALL generate the AUTH0 response according to [Table 8-5.](#page-66-1)
...
The Reader SHALL execute the failure process as defined in section [8.3.3.1](#page-62-1) if one of the following scenarios occurs:

- An error status word is returned by the User Device.
- The AUTH0 response is not formatted according to [Table 8-5.](#page-66-1)
...
- The Reader SHALL execute the key material generation procedure defined in section [8.3.1.12](#page-56-0) to calculate a cryptogramSK.
- The Reader SHALL calculate the plain\_payload using the procedure in section [8.3.1.11](#page-56-2) with cryptogramSK as key and encrypted\_payload and authentication\_tag from the cryptogram in the response message as input.
...
The Reader can continue with the expedited-Standard phase after the Expedited-Fast phase as defined in section [8.2.](#page-52-1) This is also possible after a failed or aborted trial-and-error verification process of authentication\_tag.

#### <span id="page-70-0"></span>**8.3.3.3 LOAD CERT command**

This command allows the Reader to transfer the reader\_Cert as defined in section [6.3.1.](#page-28-1) When a certificate is sent by the Reader, the Access Credential uses its associated Reader System Issuer CA certificate to verify the reader\_Cert.
...
The command data field of the LOAD CERT command message SHALL contain the compressed reader\_Cert as defined in section [6.3.1.](#page-28-1)
...
The Reader SHALL format LOAD CERT command as per section [8.3.3.3.1.](#page-70-1)
...
- The command is not formatted as per section [8.3.3.3.1.](#page-70-1)
...
The response data field of the AUTH1 response messages SHALL be the concatenation of encrypted\_payload || authentication\_tag as defined in section [8.3.1.6.](#page-55-0)
...
The Reader SHALL format the AUTH1 command as per [Table 8-9](#page-72-2) and [Table 8-10.](#page-72-3)

The Reader SHALL compute Reader signature using the Reader private key over fields in [Table](#page-75-0)  [8-12](#page-75-0) and according to section [8.3.1.2.](#page-54-1)
...
- Execute the failure process as defined in section [8.3.3.1](#page-62-1) if the reader certificate signature cannot be verified with the Reader System Issuer CA public key bound to the reader\_group\_identifier.
...
The User Device SHALL verify the reader signature as per [8.3.1.3](#page-54-2) computed over [Table 8-12](#page-75-0) using the intermediate\_reader\_PubK or reader public key from the previous step or LOAD CERT command processing and execute the failure process as defined in section [8.3.3.1](#page-62-1) if the verification fails.

The User Device SHALL compute a session symmetric key Kdh according to section [8.3.1.4](#page-54-3) using the previously received transaction\_identifier, the reader ephemeral public key and the Access Credential ephemeral private key.

The User Device SHALL execute the key material generation procedure defined in section [8.3.1.13.](#page-57-0)

### **8.3.3.4.6 Response message generation**

The User Device SHALL generate AUTH1 response according to section [8.3.3.4.2.](#page-72-0)
...
The User Device SHALL compute User Device signature using the Access Credential private key over fields in [Table 8-13](#page-76-0) and according to section [8.3.1.2.](#page-54-1)

The User Device SHALL generate encrypted\_payload and authentication\_tag according to section [8.3.1.6](#page-55-0) using the session-bound key ExpeditedSKDevice as SKDevice, the data to be returned formatted as per [Table 8-11](#page-72-1) as payload, expedited\_device\_counter as device\_counter, an empty field as aad. The device\_counter output from the procedure is used to update expedited\_device\_counter.
...
- An error status word is returned by the User Device.
- The AUTH1 response is not formatted according to [8.3.3.4.2.](#page-72-0)

The Reader SHALL execute the key material generation procedure defined in section [8.3.1.13.](#page-57-0)

The Reader SHALL compute a session symmetric key Kdh according to section [8.3.1.4](#page-54-3) using the Access Credential ephemeral public key as ePubK, the reader ephemeral private key as ePrivK, transaction\_identifier.

The Reader SHALL verify authentication\_tag and decrypt encrypted\_payload according to section [8.3.1.7](#page-55-1) using ExpeditedSKDevice as SKDevice, expedited\_device\_counter as device\_counter, an empty field as aad. The device\_counter output from the procedure is used to update expedited\_device\_counter.

The Reader SHALL execute the failure process as defined in section [8.3.3.1](#page-62-1) if authentication\_tag cannot be verified.

The Reader SHALL try to lookup the Access Credential public key using the key\_slot provided in the response.

The Reader SHALL verify the User Device signature computed over [Table 8-13](#page-76-0) if provided in the response or defer this verification to step-up phase. The Reader SHALL NOT use any information for the access decision before verification of the User Device signature.
...
This command allows the Reader to perform various operations on the Access Credential during the transaction expedited-fast, expedited-standard and step-up phase. See the signaling bitmap in section [8.3.3.4.2](#page-72-0) for details on the supported items during step-up.
...
The atomic session requirements only apply to the mailbox commands in [Table 8-16.](#page-81-0) Therefore if an atomic session has started and a subsequent exchange command does not contain tag 0xBA the atomic session will be kept open.

When an atomic session is open and a command is received that is not an EXCHANGE command, the User Device SHALL execute the failure process as defined in section [8.3.3.1.](#page-62-1)
...
The command data field of the EXCHANGE command message SHALL be the concatenation of encrypted\_payload || authentication\_tag as defined in section [8.3.1.8.](#page-55-2)
...
[Table 8-19](#page-84-0) specifies Importance levels from 0 to 4 and the associated subsampling rate:
...
The response data field of the EXCHANGE response messages SHALL be the concatenation of encrypted\_payload || authentication\_tag as defined in section [8.3.1.6.](#page-55-0)
...
The Reader SHALL generate the EXCHANGE command according to section [8.3.3.5.1.](#page-79-0)
...
When in the expedited phase, the Reader SHALL generate encrypted\_payload and authentication\_tag according to section [8.3.1.8](#page-55-2) using ExpeditedSKReader as SKReader, the command payload formatted as per [Table 8-15](#page-80-0) as payload, expedited\_reader\_counter as reader\_counter, an empty field as aad. The reader\_counter output from the procedure is used to update expedited\_reader\_counter.

When in the step-up phase, the Reader SHALL generate encrypted\_payload and authentication\_tag according to section [8.3.1.8](#page-55-2) using StepUpSKReader as SKReader, the command payload formatted as per [Table 8-15](#page-80-0) as payload, StepUp\_reader\_counter as reader\_counter, an empty field as aad. The reader\_counter output from the procedure is used to update StepUp\_reader\_counter.

#### **8.3.3.5.4 Command message processing**

The User Device SHALL execute the failure process as defined in section [8.3.3.1](#page-62-1) if one of the following scenarios occurs:

- The EXCHANGE command is not received as per sequencing flow described in section [8.2.](#page-52-1)

![](_page_85_Picture_18.jpeg)

- The EXCHANGE command is not formatted according to section [8.3.3.5.1.](#page-79-0)
...
When in the expedited phase, the User Device SHALL verify authentication\_tag and decrypt encrypted\_payload according to section [8.3.1.9](#page-55-3) using ExpeditedSKReader as SKReader, expedited\_reader\_counter as reader\_counter, an empty field as aad. The reader\_counter output from the procedure is used to update expedited\_reader\_counter.

When in the step-up phase, the User Device SHALL verify authentication\_tag and decrypt encrypted\_payload according to section [8.3.1.9](#page-55-3) using StepUpSKReader as SKReader, StepUp\_reader\_counter as reader\_counter, an empty field as aad. The reader\_counter output from the procedure is used to update StepUp\_reader\_counter.

The User Device SHALL execute the failure process as defined in section [8.3.3.1](#page-62-1) if authentication\_tag cannot be verified.

The User Device SHALL execute all the requests as defined in [Table 8-20.](#page-85-1)
...
The User Device SHALL generate EXCHANGE response according to section [8.3.3.5.2.](#page-85-0)
...
When in the expedited phase, the User Device SHALL generate encrypted\_payload and authentication\_tag according to section [8.3.1.6](#page-55-0) using the session-bound key ExpeditedSKDevice as SKDevice, the data to be returned formatted as per [Table 8-20](#page-85-1) as payload, expedited\_device\_counter as device\_counter, and empty field as aad. The device\_counter output from the procedure is used to update expedited\_device\_counter.

When in the step-up phase, the User Device SHALL generate encrypted\_payload and authentication\_tag according to section [8.3.1.6](#page-55-0) using the session-bound key StepUpSKDevice as SKDevice, the data to be returned formatted as per [Table 8-20](#page-85-1) as payload, StepUp\_device\_counter as device\_counter, and empty field as aad. The device\_counter output from the procedure is used to update StepUp\_device\_counter.
...
After sending the SessionData messages the Reader and User Device MAY exchange one or more EXCHANGE commands, see section [8.3.3.5.1,](#page-79-0) and EXCHANGE responses, see section [8.3.3.5.2.](#page-85-0)

# <span id="page-88-0"></span>**8.4.2 Request and Response messages**

If the Reader wants to request an Access Document and/or a Revocation Document, it SHALL use DeviceRequest messages as defined in [\[6\].](#page-19-7) The document type, namespaces and data element identifiers of the requested documents are defined in section [7.](#page-31-0)
...
- To optimize the size of the DeviceRequest structure the keys in the maps SHALL be replaced by a different key (note that these are integers, still encoded as text strings) according to [Table 8-21.](#page-89-1) Some of the keys are nested, shown by indentations.
- To optimize the size of the DeviceResponse structure the keys in the maps SHALL be replaced by a different key (note that these are integers, still encoded as text strings) according to [Table 8-22.](#page-89-2) Some of the keys are nested, shown by indentations.
...
- The cryptographic operations in [\[6\]](#page-19-7) clause 9.1.1.5 SHALL use the 32 bytes value for StepUpSK computed as per section [8.3.1.13](#page-57-0) as the IKM.
...
If an error occurs, the procedure in section [8.3.3.1](#page-62-1) SHALL be executed.

### <span id="page-90-0"></span>**8.4.4 APDU commands**

The SessionData messages SHALL be transmitted using the ENVELOPE and GET RESPONSE commands defined below. In this specification, the User Device indicates the supported APDU command and response sizes using the SELECT response as defined in section [10.2.1.2.](#page-93-2)
...
The EXCHANGE command and response are defined in section [8.3.3.5.](#page-78-0)
...
For 1: the User Device and the Reader SHALL support this flow according to the requirement in section [10.1.](#page-92-1)

For 2: the User Device and the Reader MAY support this flow according to the requirements in section [11.1.1.](#page-98-2)

For 3: the User Device and the Reader MAY support this flow according to the requirements in section [11.1.2.](#page-103-0)
...
When the transaction is performed using NFC, the Access Protocol commands SHALL be transmitted using NFC following the flow as defined in section [8](#page-48-0) with the following additional requirements:

Before sending the AUTH0 command as part of the Access Protocol, the Reader SHALL send the SELECT command as defined in section [10.2.1](#page-93-0) with the expedited phase AID.

The User Device can indicate that it is required to send a step-up AID SELECT command before the step-up phase. This can be useful for example if selection of a different AID is required to get access to the Access or Revocation Document. The need for a SELECT before the Step-up phase is defined by the signaling\_bitmap returned during the expedited phase in the AUTH1 response payload. If the User Device uses this mechanism to indicate the SELECT is required, the Reader SHALL send the step-up AID SELECT command before performing the step-up phase. Note that this is between step 5 and step 6 as defined in section [8.2.](#page-52-1)
...
After completing the Access Protocol as defined in section [8,](#page-48-0) the Reader SHALL send the EXCHANGE command with a Reader status sub-event defined in section [8.3.3.5,](#page-78-0) or the CONTROL FLOW command as defined in section [10.2.2](#page-95-0) when the secure channel is not available, to indicate the result of the transaction. After the response to one of these commands is received:
...
The commands defined in this section SHALL implement the requirements from section [8.3.2,](#page-60-0) with the exception of the class byte requirements for the SELECT command, which is defined in section [10.2.1.1.](#page-93-3)
...
The command data field of the SELECT command message SHALL contain the AID of the application to be selected as defined in [Table 10-3.](#page-95-1)
...
The AID TLV SHALL contain the AID of the selected application. The AIDs are defined in [Table](#page-95-1)  [10-3:](#page-95-1)
...
The Type TLV SHALL contain the application type Aliro application according to [Table 10-4.](#page-95-2) The value of the application type is coded in big endian format.
...
A vendor specific extensions TLV as described in section [8.3.3.2.3](#page-66-2) MAY be added to the proprietary information TLV in the FCI template.
...
All Aliro messages in the Bluetooth LE + UWB Aliro flow SHALL follow the Aliro message rules in section [11.9.](#page-133-0) [Figure 11-1](#page-102-0) is an informative depiction of the Bluetooth LE + UWB Aliro flow.

The Bluetooth LE + UWB Aliro flow begins with Bluetooth LE discovery and L2CAP connectionoriented channel establishment (see sections [11.4](#page-109-1) and [11.5\)](#page-109-3) between the User Device and the Reader. Then the User Device SHALL send Initiate Access Protocol Message ID (see section [11.7.3.7\)](#page-129-0) in clear over the unencrypted L2CAP connection to the Reader to trigger Reader for initiating Access Protocol.
...
The Reader SHALL send EXCHANGE command including tag 0x98 (see section [8.3.3.5.1\)](#page-79-0) to prompt the User Device to make URSK available to the UWB ranging sensor (see section [8.3.3.5\)](#page-78-0), if the cryptogram verification in the expedited-fast phase or the signature verification in the expeditedstandard phase is successful at the Reader. The User Device SHALL send EXCHANGE response indicating the success or failure status of EXCHANGE processing. The behavior of the User Device and the Reader is described in section [8.3.3.5,](#page-78-0) in case error codes are reported in EXCHANGE response. Additional EXCHANGE commands MAY be sent.
...
The Reader MAY send EXCHANGE commands according to section [8.3.3.5.](#page-78-0) If the Reader sends an EXCHANGE command, the User Device SHALL send EXCHANGE response indicating the success or failure status of EXCHANGE processing. The behavior of the User Device and the Reader is described in section [8.3.3.5,](#page-78-0) in case error codes are reported in EXCHANGE response.
...
The proximity measurement using secure UWB ranging over UWB radio is described in section [12.](#page-155-0) An ongoing UWB ranging session can be suspended and resumed according to section [11.1.1.2.](#page-102-1)

![](_page_99_Picture_10.jpeg)

#### <span id="page-100-0"></span>**11.1.1.1 Reader status reporting**

The Reader decision to change its status is out of scope of this specification. However, whenever there is a change in the Reader's status information, the Reader sends Reader Status Changed Message ID with a State Attribute ID (see section [11.7.3.3.1\)](#page-126-0) according to its capabilities indicated in Reader Status Access Protocol Completed Message ID (see section [11.7.3.4\)](#page-127-0), indicating the new Reader state.
...
Once a UWB ranging session is established, it can be active until the URSK is discarded (see section [11.1.1.4\)](#page-103-1). However, for power optimizations, either the Reader or the User Device can suspend an active ranging session.

The Reader or the User Device MAY request to suspend the current ranging session by sending Ranging Session Suspend Request Message ID (see section [11.7.2.6\)](#page-123-0). The User Device or the Reader respond with Ranging Session Suspend Response Message ID (see section [11.7.2.7\)](#page-123-1) that indicates the status of the suspend request. The User Device and the Reader SHALL suspend the current UWB ranging session only if 'Request Accepted' status is indicated in the Ranging Session Suspend Response Message ID. Otherwise, the current UWB ranging session is not suspended. Furthermore, the Reader or the User Device MAY unilaterally suspend the current ranging session by sending Ranging Message ID carrying Ranging Session Suspended Attribute ID (see section [11.7.3.2.6\)](#page-126-1). The User Device or Reader SHALL suspend the current ranging session on receiving Ranging Message ID carrying Ranging Session Suspend Attribute ID.

The Reader MAY resume a suspended UWB ranging session by sending Ranging Session Resume Request Message ID (see section [11.7.2.8\)](#page-123-2) to the User Device. The User Device MAY trigger the UWB ranging session resumption by sending Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID (see section [11.7.3.2.2\)](#page-125-0) to the Reader. Before initiating UWB ranging session resumption, the Reader SHALL confirm that there exists a non-expired URSK associated with the suspended UWB ranging session that is intended for resumption.
...
All the Aliro messages in the Bluetooth LE-Only Aliro flow SHALL follow the Aliro message rules in section [11.9.](#page-133-0) [Figure 11-2](#page-105-1) is an informative depiction of Bluetooth LE-only Aliro flow.

The Bluetooth LE-only Access flow begins with Bluetooth LE discovery and L2CAP connectionoriented channel establishment (see sections [11.4](#page-109-1) and [11.5\)](#page-109-3) between the User Device and the Reader. After the intent to request Bluetooth LE-only Access flow is determined, the User Device SHALL send Initiate Access Protocol RKE Message ID (see section [11.7.3.8\)](#page-130-1) in clear over the unencrypted L2CAP connection to the Reader to trigger Reader for initiating Access Protocol.
...
The ADV\_IND contains Advertising Address and Advertising Data (AD) shown in [Table 11-1](#page-106-4) and [Table 11-2,](#page-107-0) respectively.
...
The Bluetooth LE + UWB Aliro flow supported bit (bit 7 in byte 7 in [Table 11-2\)](#page-107-0) is set to 01h, if the Reader supports Bluetooth LE + UWB Aliro flow (see section [11.1.1\)](#page-98-2). Otherwise, the Reader does not support Bluetooth LE + UWB Aliro flow.

![](_page_107_Picture_6.jpeg)

The Bluetooth LE-Only Aliro flow supported bit (bit 6 in byte 7 in [Table 11-2\)](#page-107-0) is set to 01h, if the Reader supports Bluetooth LE-Only Aliro flow (see section [11.1.2\)](#page-103-0). Otherwise, the Reader does not support Bluetooth LE-Only Aliro flow.
...
truncated\_reader\_group\_identifier field is the first 8 octets of the reader\_group\_identifier (see section [6.2\)](#page-27-2). truncated\_reader\_group\_sub\_identifier is the first 2 octets of the reader\_group\_sub\_identifier (see section [6.2\)](#page-27-2).
...
Dynamic Tag field carries the dynamic tag data generated as in section [11.3.1.](#page-108-0)
...
The Reader begins by sending ADV\_IND with Aliro service UUID and the advertising payload in section [11.3.](#page-106-3) The Reader SHALL be in the advertising state with its filtering policy set to accept all connection requests.
...
An example filter policy can use Aliro service UUID along with advertisement address and/or truncated\_reader\_group\_identifier and/or truncated\_reader\_group\_sub\_identifier and/or Dynamic Tag (see section [11.4.1\)](#page-109-4).
...
- Read the characteristic by UUID with the UUID\_SPSM\_ALIRO\_BLE\_UWB\_PROTOCOL\_VERSION (see [Table 11-3](#page-110-3) and [Table](#page-111-0)  [11-4\)](#page-111-0). The SPSM value uses big-endian byte order.
- Write the characteristics by UUID for UUID\_DEVICE\_ALIRO\_BLE\_UWB\_PROTOCOL\_VERSION (see [Table 11-5](#page-111-1) and [Table](#page-112-1)  [11-6\)](#page-112-1).
...
[Table 11-10](#page-115-1) shows the Aliro message format. All multi-octet integer fields in Aliro message are encoded in big-endian order.

<span id="page-115-1"></span>**Field Length (Octets) Value Description** Protocol Header 1 See [Table](#page-116-0)  [11-11](#page-116-0) Bits B5:B0 indicate Protocol Type. Bits B7:B6 are RFU and set to 0. Message ID 1 See [Table](#page-116-0)  [11-11](#page-116-0) Identifier of the Message. Length 2 Variable Indicates length in octets of the Payload field. Payload Variable Variable Contains data corresponding to the Protocol Type and Message ID.

**Table 11-10 – Aliro message format**

When Protocol Type is equal to AP then the Payload field contains the commands listed in section [8.3](#page-53-0) and [8.4.](#page-87-0)

When Protocol Type is equal to UWB Ranging Service and Supplementary Service then the Payload field carries one or more Attributes IDs (see sections [11.7.2](#page-117-1) and [11.7.4,](#page-130-0) respectively). When the Protocol Type is equal to Notification then the Payload field SHALL carry only one Attribute ID (see section [11.7.3\)](#page-124-0) to avoid presence of conflicting Attributes in one Notification. This rule has an exception: if the Payload field includes the General Error Attribute ID, the Reader Descriptor Attribute ID MAY also be included. For any other Attribute ID in the Payload field, only one Attribute ID is permitted, for Notification Protocol Type. When Protocol Type is equal to 3rd Party App Payload field content is defined in section [11.7.5.](#page-131-0)
...
<span id="page-117-2"></span>The Attribute IDs for UWB Ranging Service Protocol Type are listed in [Table 11-13.](#page-117-2)
...
The Attribute Value field carries session identifier, which is the least significant four octets of the Transaction Identifier field (see [Table 8-4\)](#page-65-2).
...
In this specification, User Device and Reader shall support default hopping sequence (see section [17\)](#page-186-0). The AES-based hopping sequence defined in [\[2\]](#page-19-5) shall not be used in this specification.
...
![](_page_123_Picture_28.jpeg)

### <span id="page-124-0"></span>**11.7.3 Notification Protocol Type**

#### <span id="page-124-1"></span>**11.7.3.1 Event Message ID**

The Event Message ID is sent by the User Device and the Reader. The Payload field carries the Attribute IDs in [Table 11-15.](#page-124-3)
...
The Attribute Length field is set to one octet. The Attribute Value field carries the reason code enumerated in [Table 11-16.](#page-124-4) Only Unknown Error reason code and Resource Unavailable SHALL be sent if the BleSK is not available for encryption. If the BleSK is available for encryption, any reason code is allowed to be sent.
...
The Attribute Length field is set to variable octets. The Attribute Value field is enumerated in [Table](#page-125-1)  [11-17.](#page-125-1)
...
<span id="page-125-2"></span>The Payload field carries the Attribute IDs in [Table 11-18.](#page-125-2)
...
The Payload field SHALL carry State Attribute IDs defined in [Table 11-19](#page-126-2) and [Table 11-20,](#page-127-1) respectively.
...
The Attribute Length field is set to two octets. The bits [B15:B8] in the Attribute Value field indicate the operation source information enumerated in [Table 11-20.](#page-127-1)

![](_page_126_Picture_22.jpeg)

The bits [B7:B0] in the Attribute Value field indicate the Reader status information enumerated in the second byte field when the first byte field is equal to 0x01 in [Table 8-18.](#page-83-0)
...
The Attribute Length field is set to two octets. The bits [B15:B8] in the Attribute Value field indicate Reader capabilities enumerated in [Table 11-22.](#page-128-0) The bits [B7:B0] in the Attribute Value field indicate the Reader status information enumerated in the second byte field when the first byte field is equal to 0x01 in [Table 8-18.](#page-83-0)

A Reader SHALL support Unsolicited Reader Status Reporting with a value of either 1 or 2 when it supports, Bluetooth LE-Only Aliro flow (see section [11.1.2\)](#page-103-0) or Bluetooth LE + UWB Aliro flow (see section [11.1.1\)](#page-98-2).
...
The Attribute Length field is set to one octet. The Attribute Value field is enumerated in [Table](#page-129-1)  [11-24.](#page-129-1)
...
<span id="page-129-2"></span>The Payload field carries Proprietary Information Attribute ID in [Table 11-25.](#page-129-2)
...
The Payload carries the Proprietary Information Attribute ID from [Table 11-25.](#page-129-2) Other Attribute IDs are RFU.

# <span id="page-130-0"></span>**11.7.4 Supplementary Service Protocol Type**

#### **11.7.4.1 Attribute ID**

<span id="page-130-2"></span>Attribute IDs for Supplementary Service Protocol Type are listed in [Table 11-26.](#page-130-2)
...
The Aliro messages with Protocol Type "AP" SHALL have their payloads encrypted and authenticated as per section [8.](#page-48-0)

The Aliro messages with Protocol Type "UWB Ranging Service", "Notification", "Supplementary Service", and "3rd Party App" SHALL have their payloads encrypted and authenticated as per sections [11.8.1](#page-132-1) and [11.8.2.](#page-132-2)

# <span id="page-132-1"></span>**11.8.1 Session Key Derivation**

The 32 bytes key BleSKReader SHALL be derived as per section [8.3.1.5](#page-54-0) using BleSK (computed as per as per section [8.3.1.12](#page-56-0) or section [8.3.1.13\)](#page-57-0) as input\_key\_material, "BleSKReader" (utf8-encoded string) as info, 32 as key\_material\_length, salt as below.

The 32 bytes key BleSKDevice SHALL be derived as per section [8.3.1.5](#page-54-0) using BleSK (computed as per section [8.3.1.12](#page-56-0) or section [8.3.1.13\)](#page-57-0) as input\_key\_material, "BleSKDevice" (utf8-encoded string) as info, 32 as key\_material\_length, salt as below.

The "salt" input of the key derivation contains: the list of protocol versions supported by the Reader referenced as "Supported ALIRO BLE UWB Protocol Version" in [Table 11-7](#page-112-0) denoted as reader\_supported\_versions below, the protocol version selected by the User Device referenced as "Selected ALIRO BLE UWB Protocol Version" in [Table 11-6](#page-112-1) denoted as user\_device\_selected\_version below. The "salt" is generated as follow:
...
[Table 11-27](#page-134-0) enumerates the various Message IDs and their corresponding permitted Message ID reply. [Table 11-28](#page-137-0) enumerates Message IDs that do not have a corresponding Message ID reply.
...
On receiving any Message ID in first column of [Table 11-27,](#page-134-0) the receiver SHALL set for this Message ID, response timer equal to responseTimeout, and start its countdown. On receiving any Message ID in [Table 11-28,](#page-137-0) the receiver SHALL NOT set response timer. The responseTimeout is set to 1500 ms in this specification. [Figure 11-4](#page-139-1) is informative depiction of receiver side Aliro message rules.

While the response timer is not expired at the receiver:

- 1. The receiver SHALL send the Message ID reply in the second column of [Table 11-27](#page-134-0) in response to the corresponding received Message ID in the first column in [Table 11-27.](#page-134-0) The receiver SHALL delete the response timer after transmitting Message ID reply other than Event Message ID carrying Busy Attribute ID in response to the corresponding received Message ID in the first column of [Table 11-27.](#page-134-0)
- 2. The receiver sends Event Message ID carrying Busy Attribute ID in response to the corresponding received Message ID in the first column of [Table 11-27,](#page-134-0) if the receiver needs more time beyond the response timer for processing. The receiver SHALL reset the response timer to responseTimeout after transmitting Event Message ID carrying Busy Attribute ID in response to the corresponding received Message ID in the first column of [Table 11-27.](#page-134-0) The number of times a receiver can send Event Message ID carrying Busy Attribute ID in response to a Message ID is implementation specific and outside scope of this specification.

The receiver can incur an exception such that response timer is expired, and the receiver is unable to transmit the Message ID reply in the second column of [Table 11-27](#page-134-0) in response to the corresponding received Message ID in the first column of [Table 11-27.](#page-134-0) In this case, the transmitter of the Message ID SHALL initiate Bluetooth LE connection teardown according to section [11.6.](#page-114-0)
...
On sending any Message ID in the first column of [Table 11-27,](#page-134-0) the transmitter SHALL set for this Message ID, response timer equal to responseTimeout, and start its countdown. On sending any Message ID in [Table 11-28,](#page-137-0) the transmitter SHALL NOT set response timer. The responseTimeout is set to 1500 ms in this specification. [Figure 11-5](#page-140-1) is informative depiction of transmitter side Aliro message rules.

While the response timer is not expired at the transmitter:

- 1. The transmitter SHALL delete the response timer after receiving the Message ID reply other than the Event Message ID carrying Busy Attribute ID in response to the corresponding transmitted Message ID in the first column of [Table 11-27.](#page-134-0)
- 2. The transmitter SHALL initiate Bluetooth LE connection teardown after receiving the Event Message ID carrying General Error Attribute ID in response to the corresponding transmitted Message ID in the first column of [Table 11-27.](#page-134-0)
...
These delays at the Reader and the User Device yield out-of-order Aliro message exchanges that do not conform to [Table 11-27.](#page-134-0) The out-of-order Aliro message exchanges scenarios are listed in [Table](#page-141-0)  [11-29](#page-141-0) and its resolutions are described in this section.
...
[Figure 11-6](#page-143-0) illustrates the out-of-order index 1 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Setup M1 Message ID and receives Ranging Message ID carrying Initiate Ranging Session Attribute ID that is not an expected reply according to [Table 11-27.](#page-134-0)

User Device sends Ranging Message ID carrying Initiate Ranging Session Attribute ID and receives Ranging Session Setup M1 Message ID that is an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL ignore and discard the Ranging Message ID carrying Initiate Ranging Session Attribute ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Session Setup M1 Message ID it sent.
...
[Figure 11-7](#page-144-0) illustrates the out-of-order index 5 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Resume Request Message ID and receives Ranging Message ID carrying Initiate Ranging Session Attribute ID that is not an expected reply according to [Table 11-27.](#page-134-0)

User Device sends Ranging Message ID carrying Initiate Ranging Session Attribute ID and receives Ranging Session Resume Request Message ID that is not an expected reply according to [Table](#page-134-0)  [11-27.](#page-134-0)

In this case, the Reader SHALL discard Ranging Session Resume Request Message ID it sent and delete the corresponding responseTimeout set according to section [11.9.2.](#page-139-0) The Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Message ID carrying Initiate Ranging Session Attribute ID it received.

The User Device SHALL ignore and discard Ranging Session Resume Request Message ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Message ID carrying Initiate Ranging Session Attribute ID it sent.
...
[Figure 11-8](#page-144-1) illustrates the out-of-order index 6 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Resume Request Message ID and receives Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID that is not an expected reply according to [Table 11-27.](#page-134-0)

User Device sends Ranging Session Suspend Request Message ID and receives Ranging Session Resume Request Message ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL discard Ranging Session Resume Request Message ID it sent and delete the corresponding responseTimeout set according to section [11.9.2.](#page-139-0) The Reader SHALL follow section [11.9.1](#page-138-0) and to [Table 11-27](#page-134-0) in responding to Ranging Session Suspend Request Message ID it received.
...
[Figure 11-9](#page-145-0) illustrates the out-of-order index 7 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Resume Request Message ID and receives Ranging Session Suspend Request Message ID that is not an expected reply according to [Table 11-27.](#page-134-0)

User Device sends Ranging Session Suspend Request Message ID and receives Ranging Session Resume Request Message ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL discard Ranging Session Resume Request Message ID it sent and delete the corresponding responseTimeout set according to section [11.9.2.](#page-139-0) The Reader SHALL follow section [11.9.1](#page-138-0) and to [Table 11-27](#page-134-0) in responding to Ranging Session Suspend Request Message ID it received.

The User Device SHALL ignore and discard Ranging Session Resume Request Message ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Session Suspend Request Message ID it sent.
...
[Figure 11-10](#page-145-1) illustrates the out-of-order index 8 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Resume Request Message ID and receives Ranging Message ID carrying Ranging Session Suspended Attribute ID that is not an expected reply according to [Table](#page-134-0)  [11-27.](#page-134-0)

User Device sends Ranging Message ID carrying Ranging Session Suspended Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

In this case, the Reader SHALL ignore and discard Ranging Message ID carrying Ranging Session Suspended Attribute ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Session Resume Request Message ID it sent.

The User Device SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Session Resume Request Message ID it received.
...
[Figure 11-11](#page-146-0) illustrates the out-of-order index 9 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Suspend Request Message ID and receives Ranging Message ID carrying Initiate Ranging Session Attribute ID that is not an expected reply according to [Table 11-27.](#page-134-0)

![](_page_145_Picture_16.jpeg)

User Device sends Ranging Message ID carrying Initiate Ranging Session Attribute ID and receives Ranging Session Suspend Request Message ID that is not an expected reply according to [Table](#page-134-0)  [11-27.](#page-134-0)

In this case, the Reader SHALL discard Ranging Session Suspend Request Message ID it sent and delete the corresponding responseTimeout set according to section [11.9.2.](#page-139-0) The Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Message ID carrying Initiate Ranging Session Attribute ID it received.

The User Device SHALL ignore and discard Ranging Session Suspend Request Message ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Message ID carrying Initiate Ranging Session Attribute ID it sent.
...
[Figure 11-12](#page-147-0) illustrates the out-of-order index 10 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Suspend Request Message ID and receives Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID that is not an expected reply according to [Table 11-27.](#page-134-0)

User Device sends Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID and receives Ranging Session Suspend Request Message ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL discard Ranging Session Suspend Request Message ID it sent and delete the corresponding responseTimeout set according to section [11.9.2.](#page-139-0) The Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID it received.

The User Device SHALL ignore and discard Ranging Session Suspend Request Message ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID it sent.
...
[Figure 11-13](#page-147-1) illustrates the out-of-order index 11 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Suspend Request Message ID and receives Ranging Message Session Suspend Request ID that is not an expected reply according to [Table 11-27.](#page-134-0)

User Device sends Ranging Session Suspend Request Message ID and receives Ranging Session Suspend Request Message ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL discard Ranging Session Suspend Request Message ID it sent and delete the corresponding responseTimeout set according to section [11.9.2.](#page-139-0) The Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Session Suspended Message ID it received.

The User Device SHALL ignore and discard Ranging Session Suspend Request Message ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Session Suspend Request Message ID it sent.
...
[Figure 11-14](#page-148-0) illustrates the out-of-order index 12 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Session Suspend Request Message ID and receives Ranging Message ID carrying Ranging Session Suspended Attribute ID that is not an expected reply according to [Table](#page-134-0)  [11-27.](#page-134-0)

User Device sends Ranging Message ID carrying Ranging Session Suspended Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

In this case, the Reader SHALL ignore and discard Ranging Message ID carrying Ranging Session Suspended Attribute ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Session Suspend Request Message ID it sent.

The User Device SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Session Suspend Request Message ID it received.
...
[Figure 11-15](#page-148-1) illustrates the out-of-order index 13 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Message ID carrying Ranging Session Suspended Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

User Device sends Ranging Message ID carrying Initiate Ranging Session Attribute ID and receives Ranging Message ID carrying Ranging Session Suspended Attribute ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Message ID carrying Initiate Ranging Session Attribute ID it received.

The User Device SHALL ignore and discard Ranging Message ID carrying Ranging Session Suspended Attribute ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Message ID carrying Initiate Ranging Session Attribute ID it sent.
...
[Figure 11-16](#page-149-0) illustrates the out-of-order index 14 scenario in [Table 11-29.](#page-141-0)

![](_page_148_Picture_16.jpeg)

Reader sends Ranging Message ID carrying Ranging Session Suspended Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

User Device sends Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID and receives Ranging Message ID carrying Ranging Session Suspended Attribute ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID it received.

The User Device SHALL ignore and discard Ranging Message ID carrying Ranging Session Suspended Attribute ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID it sent.
...
[Figure 11-17](#page-150-0) illustrates the out-of-order index 15 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Message ID carrying Ranging Session Suspended Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

User Device sends Ranging Session Suspend Request Message ID and receives Ranging Message ID carrying Ranging Session Suspended Attribute ID that is not an expected reply according to [Table](#page-134-0)  [11-27.](#page-134-0)

In this case, the Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Session Suspend Request Message ID it received.

The User Device SHALL ignore and discard Ranging Message ID carrying Ranging Session Suspended Attribute ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Session Suspend Request Message ID it sent.
...
[Figure 11-18](#page-151-0) illustrates the out-of-order index 17 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

User Device sends Ranging Message ID carrying Initiate Ranging Session Attribute ID and receives Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Message ID carrying Initiate Ranging Session Attribute ID it received.

The User Device SHALL ignore and discard Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Message ID carrying Initiate Ranging Session Attribute ID it sent.
...
[Figure 11-19](#page-151-1) illustrates the out-of-order index 18 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

User Device sends Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID and receives Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL follow section [11.9.1](#page-138-0) and [Table 11-27](#page-134-0) in responding to Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID it received.

The User Device SHALL ignore and discard Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID it receives and continues its responseTimeout countdown according to section [11.9.2](#page-139-0) for Ranging Message ID carrying Initiate Ranging Session Resume Attribute ID it sent.
...
[Figure 11-20](#page-152-1) illustrates the out-of-order index 19 scenario in [Table 11-29.](#page-141-0)

Reader sends Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)

![](_page_151_Picture_15.jpeg)

User Device sends Ranging Session Suspend Request Message ID and receives Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attributed ID that is not an expected reply according to [Table 11-27.](#page-134-0)

In this case, the Reader SHALL ignore and discard Ranging Session Suspend Request Message ID it received.

The User Device SHALL discard Ranging Session Suspend Request Message ID it sent and delete its corresponding responseTimeout countdown set according to section [11.9.2](#page-139-0) for Ranging Session Suspend Request Message ID it sent.

User Device behavior on receiving Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attribute ID is described in section [11.1.1.3.](#page-102-2)
...
Reader sends Ranging Message ID carrying Secure Ranging Over UWB Radio Failed Attribute ID and this Message ID has no corresponding response in [Table 11-28.](#page-137-0)
...
The MAC layer of the UWB-related functionality of Aliro is defined in section 20 in [\[2\].](#page-19-5) Here we describe the extensions to section 20 in [\[2\]](#page-19-5) that are relevant to Aliro. See section [11.11](#page-154-1) for considerations while referring to [\[2\].](#page-19-5)
...
The UWB ranging session parameters are agreed upon between the initiator and the responder through ranging session setup shown in [Figure 12-2](#page-160-0) and described below.
...
The Reader manufacturer should use a hardware mechanism like a write latch to disable changes to the Credential Issuer root certificate or Credential Issuer public key (see section [6.5](#page-29-1) for details) except when the device is in provisioning mode.
...
Random number generation is described in section [8.3.1.1.](#page-53-2)

# <span id="page-184-1"></span>**16.1.6 Traceable identifiers**

Data that could be used to track an individual or specific User Device, such as the ID (see section [7.3.2\)](#page-36-1) should have confidentiality protection when in transit.
...
The mailbox (see section [8.3.1.15\)](#page-59-1) on User Device and Reader should not be accessible by any other means or interfaces than defined in this specification. Protection of data at rest in mailboxes on the User Device or reader is beyond the scope of the Access Control protocol. These protections are provided by the execution environments in which the mailboxes are instantiated (e.g. secure element, operating system). It is recommended that implementers protect data stored in a remote mailbox (e.g. through encryption and integrity protection) to limit any dependency on the security assurance of the mailbox itself.
...
Long term asymmetric keys \*PrivK should be stored using isolated processing approaches (see section [16.1.4\)](#page-183-5). They should not be accessible outside the isolated processing environment.
...
Section [7.4](#page-43-0) for credential verification

Section [8.3.1](#page-53-1) for expedited-phase security

Section [8.4.3](#page-89-0) step-up security

Section [11.3.1](#page-108-0) Bluetooth LE tag generation

Section [11.8](#page-132-0) Access control message security

Section [13.1](#page-164-1) certificate requirements
