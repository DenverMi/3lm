# thread_0269: Re: 【Bluetooth Test Request】[Internal]Alinco [ID] Bluetooth logo test RF PHY at HQ

- Message count: 1
- Source JSON: `thread_0269.json`
...
**From:** Itsuo Sakai
**To:** =?utf-8?B?TGVvIENob3Uo5ZGo5bGV5byYKQ==?= , "Toshitaka Mochizuki" , =?utf-8?B?TmluZyBMaW4o5p6X6Iq35a+nKQ==?=
**Attachments:** Re-reviced_RF-PHY_TestPlan_ICS_IXIT(AT).xls
...
We’re sorry that the Outpot power isn't meet the RF PHY test requirement.
...
I have install the [ID] driver and fallow the settings to set up (after install the [ID] driver LED
is light on when connect DUT)， but DUT still not working and shows error message when running LE HCI commands:

Please refer the 1020 log files, it seems “Failed call to teGetBuildId” and failed to running tester HCI
commands:
...
I have received the [ID] DUT, But the DUT cannot power on propyl , according to the [ID] test sample
information, to hold power switch for 3 seconds to start the device, but once the button is released then the DUT is auto power off:
